...
# Criar tabelas de roteamento para subredes publicas e privadas
    ...
    public_routetableid=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$vpcid --output text --query 'RouteTables[*].RouteTableId')
    echo RouteTableId=$public_routetableid

    echo "Alterando Tag Name da tabela de roteamento para 'Tabela de Roteamento Publica'"
    aws ec2 create-tags --resources $public_routetableid --tags Key=Name,Value="Tabela de Roteamento Publica"

    echo "Criando tabela de roteamento para subredes privadas"
    private_routetableid=$(aws ec2 create-route-table --vpc-id $vpcid --output text --query 'RouteTable.RouteTableId')

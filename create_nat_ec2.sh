...
echo; echo =================================================================================; echo
    ...
    ec2_InstanceId_NAT1=$(aws --profile ${profile} ec2 run-instances --image-id ${image_id} --key-name ${key_name} --security-group-ids ${security_group_NAT1} --instance-type ${instance_type} --subnet-id ${subnetid_1} --private-ip-address ${private_ip_address_NAT1} --associate-public-ip-address --user-data file://nat1-user-data.sh --output text --query 'Instances[*].InstanceId')
    ...
    aws --profile ${profile} ec2 create-tags --resources ${ec2_InstanceId_NAT1} --tags Key=Name,Value="nat1.${vpc_name}"

    echo "Alterando parâmetro source-dest-check para no"
    aws --profile ${profile} ec2 modify-instance-attribute --instance-id ${ec2_InstanceId_NAT1} --no-source-dest-check

    echo "Adicionando a instância NAT1 na tabela de roteamento privada para destinos 0.0.0.0/0"
    aws --profile ${profile} ec2 create-route --route-table-id ${private1_routetable_id} --destination-cidr-block 0.0.0.0/0 --instance-id ${ec2_InstanceId_NAT1}
...
echo; echo =================================================================================; echo
    ...
    aws --profile ${profile} ec2 create-tags --resources ${ec2_InstanceId_NAT3} --tags Key=Name,Value="nat3.${vpc_name}"

    echo "Alterando parâmetro source-dest-check para no"
    aws --profile ${profile} ec2 modify-instance-attribute --instance-id ${ec2_InstanceId_NAT3} --no-source-dest-check

    echo "Adicionando a instância NAT3 na tabela de roteamento privada para destinos 0.0.0.0/0"
    aws --profile ${profile} ec2 create-route --route-table-id ${private2_routetable_id} --destination-cidr-block 0.0.0.0/0 --instance-id ${ec2_InstanceId_NAT3}
...
echo; echo =================================================================================; echo
    ...
    aws --profile ${profile} ec2 create-tags --resources ${ec2_InstanceId_NAT5} --tags Key=Name,Value="nat5.${vpc_name}"

    echo "Alterando parâmetro source-dest-check para no"
    aws --profile ${profile} ec2 modify-instance-attribute --instance-id ${ec2_InstanceId_NAT5} --no-source-dest-check

    echo "Adicionando a instância NAT5 na tabela de roteamento privada para destinos 0.0.0.0/0"
    aws --profile ${profile} ec2 create-route --route-table-id ${private3_routetable_id} --destination-cidr-block 0.0.0.0/0 --instance-id ${ec2_InstanceId_NAT5}
...
echo; echo =================================================================================; echo
    ...
    ec2_InstanceId_NAT7=$(aws --profile ${profile} ec2 run-instances --image-id ${image_id} --key-name ${key_name} --security-group-ids ${security_group_NAT7} --instance-type ${instance_type} --subnet-id ${subnetid_7} --private-ip-address ${private_ip_address_NAT7} --associate-public-ip-address --user-data file://nat7-user-data.sh --output text --query 'Instances[*].InstanceId')
    ...
    aws --profile ${profile} ec2 create-tags --resources ${ec2_InstanceId_NAT7} --tags Key=Name,Value="nat7.${vpc_name}"

    echo "Alterando parâmetro source-dest-check para no"
    aws --profile ${profile} ec2 modify-instance-attribute --instance-id ${ec2_InstanceId_NAT7} --no-source-dest-check

    echo "Adicionando a instância NAT7 na tabela de roteamento privada para destinos 0.0.0.0/0"
    aws --profile ${profile} ec2 create-route --route-table-id ${private4_routetable_id} --destination-cidr-block 0.0.0.0/0 --instance-id ${ec2_InstanceId_NAT7}
